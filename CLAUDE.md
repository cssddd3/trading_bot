...
- **9-17 실사고: /resume이 안 먹히는 무한 halt.** 사용자가 토스 앱에서 003350·HOOD·000990을
  직접 매도 → 대사(1시간마다)가 "원인불명 유출"로 halted=True + 봇소유 스탑은 그대로 방치.
  /resume은 halted만 False로 바꿀 뿐 불일치 원인을 안 고쳐서, 최대 1시간 내 다음 대사에서
  다시 halted=True — 겉보기엔 "/resume이 안 먹힘". 수리: `self._sync_mismatch`(불일치 종목
  집합)를 reconcile()이 채우고, `/resume`은 이게 비어있을 때만 실제로 재개(비어있지 않으면
  /sync 안내), **`/sync`** 신규 명령이 실보유에 맞춰 장부 보정(0이면 삭제+추정손익 기록,
  부분감소면 수량만 조정) + 봇소유 잔여 스탑 정리(broker.cancel_stops_for) + 자동 재개.
  비서 컨텍스트에도 halt 원인(_sync_mismatch)을 노출 — "왜 멈췄어" 질문에 /sync로 바로 안내.
  **원칙**: 실보유가 장부보다 많은 쪽(사용자 추가매수)은 여전히 안 건드림(기존 보유 불가침).
...
| ATR 래칫 출구 (cap 1.25/1.5/2.0x) | OOS 기대값·MC 전 구간 악화 (예: 손실확률 2.4%→55.8%) | 중간 승자를 잘라 대박 의존도만 높임 (Davey 567k백테스트 경고 재현) |
...
**9-09 트라이얼 실행 결과** (research/trial_2026-09-09.py, 4년 스캐너 유니버스 908 진입
후보, OOS 340건 — 기존 423건과 창/이벤트파일 차이로 약간 다르나 동일 성격):
- ✅ **채택: 타임스탑** (이슈 #15 → config.TIME_STOP, run_dryrun.py process()). 15거래일
  후 진입ATR×1.0도 못 오르면 익일 시가 청산. 밴드 이탈 신호가 있으면 항상 그게 우선
  (타임스탑은 강등 조건일 뿐). OOS 기대값 +3.61%(베이스라인 +3.68%) MC손실확률 2.6%
  (베이스라인 2.4%) — 거의 불변 + 14%(127/908) 조기청산으로 자금 회전. vb 유산 포지션
  같은 "단타 진입이 장기 표류" 패턴의 구조적 해법
- ✅ **채택: 변동성체제 사이징** (이슈 #14 → config.VOL_REGIME, `_refresh_vol_regime`/
  `_is_high_vol_regime`, virtual_buy에서 budget_krw×size_mult). KODEX200(KR)/SPY(US)
  20일 실현변동성이 과거 500거래일 80퍼센타일 초과 시 신규 진입 절반 사이즈 — **스킵
  아님, 사이징만**이라 st/스캐너 모두 적용해도 OOS 30건 하한 불변. 결과: MC손실확률
  2.4%→0.9%, MDD최악5% -85.8%→-67.7%, p5수익률 +87%→+173% (기대값은 +3.68→+2.78%로
  다소 감소 — 사이징 트레이드오프, 예상된 결과). 지수 캔들은 하루 1회 API로 갱신
  (refresh_hours=20), 실패해도 직전 판정 유지(open-fail)
- ❌ **기각: ATR 래칫** (이슈 #13, 원장 등재). cap 1.25/1.5/2.0x 전 구간에서 기대값·MC
  모두 베이스라인보다 악화 (예: cap1.25 손실확률 2.4%→55.8%). Davey 567k백테스트 경고
  ("타이트닝 오버레이는 대개 기대값을 훼손") 재확인 — 상위5%보존율이 오히려 급등해
  "중간 승자를 잘라 대박 의존도를 높이는" 역효과 확인. 재탕 금지
- 메타라벨링(#16)은 위 출구 규칙이 확정된 뒤 착수 (라벨이 출구에 의존) — 대기 중
...
- **스카우트**: 랭킹 상위 100 풀(1콜) → 규칙 필터(레버리지/인버스·신규상장 60일·경고 제외)
  ...
  **9-17 발견 (사용자 관찰에서 출발)**: scout_picks.csv 91개 고유 추천 사후채점 — 73%
  하락, 평균 -3.4%(같은 기간 KODEX200 -3.6%였지만 개별 낙폭은 그 2~8배, SPY -0.71%인데
  US픽 다수 -10%대 — 지수 하락으로 설명 안 됨). 절반 가까이 사유가 "급등"이었음 — 후보
  풀 자체가 '오늘 거래대금 급증'(=이미 크게 움직인) 종목이라 구조적으로 vb/뉴스모멘텀
  기각 사유("급등 추격 = 꼭지 매수")와 같은 패턴에 걸림. 매수를 안 하니 실손실은 없지만
  "관심종목=좋아 보인다"는 신호로 읽혀 신뢰 문제. 수리: SYSTEM 프롬프트에 이 추적결과와
  "당일 +8%↑ 원칙적 감점" 명시, 후보 카드에 change_rate 앞배치+⚠️표기, 마감리포트
  성적표에 상승비율 자동표기(+40%미만 시 경고). 백테스트 게이트 대상 아님(LLM 재량) —
  향후 축적되는 scout_picks.csv로 계속 관찰만.
  **구조적 해법 (같은 턴 후속, 사용자 질문 "급등주 쫓는 게 의미 있냐"에서 출발)**:
  스캐너가 예전에 정확히 같은 문제("당일 거래대금 상위" 유니버스 -4.5%/건 기각 →
  "60일 평균" 유니버스로 교체해 검증 통과)를 겪었던 해법을 스카우트에도 적용.
  config.SCOUT["steady_slots"]=8(KR만) — max_candidates 20개 중 8개는 오늘 랭킹이
  아니라 data/scanner/universe_seed.json(스캐너의 60일평균 유니버스)에서 날짜시드
  랜덤 샘플링 + 실제 등락률 조회(get_candles count=2) + 동일 품질필터(_quality_ok로
  spike/steady 공용화). 후보 카드에 🔥오늘상위/🐢안정유니버스 태그, SYSTEM 프롬프트가
  "재료 있으면 안정유니버스 우선 고려"라고 명시. US는 스캐너 시드가 없어 미적용(추후
  이슈).
