...
SCOUT: dict = {
    ...
    "steady_slots": 8,                # 9-17: max_candidates 중 이만큼은 '오늘 급등' 랭킹이
                                     # 아니라 data/scanner/universe_seed.json(60일 평균
                                     # 거래대금 상위 — 스캐너가 이미 검증한 유니버스)에서
                                     # 뽑는다. 스카우트 추천 91건 사후채점 결과 73% 하락 +
                                     # 40%가 '급등'을 사유로 명시 — 후보 풀이 전부 '오늘
                                     # 이미 크게 움직인' 종목이라 스캐너가 예전에 기각한
                                     # 당일-거래대금 유니버스와 같은 함정. 스캐너가 60일
                                     # 평균으로 바꿔서 검증 통과했던 해법을 여기도 적용
    "max_picks": 5,                  # LLM이 고를 최대 종목 수 (시장별)
...
# 전환 스캐너 (2026-08-31 승격, 9-02 유니버스 200 확대 재검증 통과):
# 유니버스 = **60일 평균 거래대금 상위 top_n** (당일 기준 아님 — 당일 spike는 기각).
# 4년 검증: 상위 200 OOS 423건 +2.49%/건, 1/2 분할 MC 손실확률 10.7% ✅ (상위 100은 13.6%).
# '분할'이 승격 조건 — position_frac 제거 금지 (전액 몰빵은 MC 30%+ 기각 조건).
SCANNER: dict = {
    "enabled": True,
    "top_n": 200,                    # 60일 평균 거래대금 상위 몇 개를 스캔할지
    "pool_n": 300,                   # 실시간 랭킹에서 받아올 후보풀 (여기서 60일 평균 재랭킹)
